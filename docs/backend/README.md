...
- **Validación automática** de informes .docx y .pdf usando IA (Groq API - LLaMA 3.3 70B)
- **Flujo de estados**: Enviado → Validando → Observado → Revisión Docente → Aprobado/Rechazado
...
- **Procesamiento de documentos**: python-docx + pypdf
...
backend/
├── apps/
│   ├── presentacion/   # Capa de presentación (views por flujo)
│   ├── negocio/        # Capa de negocio (servicios)
│   ├── datos/          # Capa de datos (repositorios ORM)
│   ├── core/           # Compatibilidad y URLs
...
DEBUG=True
DATABASE_URL=sqlite:////home/usuario/T.A-Arquitectura-de-software/database/db.sqlite3
...
- usuario (FK)
- nombre_archivo
- contenido (texto extraído del .docx o .pdf)
- fecha_registro
- estado (enviado/validando/observado/revision_docente/rechazado/aprobado/completado)

...
| `nombre_archivo` | CharField(255) | NOT NULL | Nombre del archivo .docx o .pdf |
...
num_obs = informe.observaciones.count()
...
usuario = Usuario.objects.get(codigo='2021101234')
informes = Informe.objects.filter(usuario=usuario).prefetch_related('observaciones')

for informe in informes:
    print(f"{informe.nombre_archivo}: {informe.observaciones.count()} observaciones")
```
